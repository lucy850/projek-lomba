...
const ekspresiKucing = { // Catatan: Variabel ini masih 'ekspresiKucing' meskipun untuk gajah. Tidak mempengaruhi fungsionalitas.
    normal: "gajah 1.png",
    love: "gajah seneng.png",
    sad: "gajah sedih.png"
};

const itemData = {
    hair: [
        { name: "Long", price: 30, image: "long gajah.png" },
        { name: "Curly", price: 30, image: "curly gajah.png" },
        { name: "Short", price: 30, image: "short gajah.png" }
    ],
    accessories: [
        { name: "Elephant Scarf", price: 30, image: "syal gajah.png" },
        { name: "Elephant Hat", price: 30, image: "hat gajah.png" },
        { name: "Elephant Necklace", price: 30, image: "kalung gajah.png" }
    ],
    playTools: [
...
function showExpression(type, duration = 0) {
    ...
    const terpasang = JSON.parse(sessionStorage.getItem("itemTerpasang")) || {}; // DIUBAH KE sessionStorage
...
function feedPet() {
    ...
    if (typeof coins !== 'undefined' && coins >= hargaMakanan) {
        coins -= hargaMakanan;
        sessionStorage.setItem("coins", coins); // DIUBAH KE sessionStorage
        updateCoinDisplay();
...
function lepasItem(tipe) {
    ...
    sessionStorage.setItem("itemTerpasang", JSON.stringify(terpasang)); // DIUBAH KE sessionStorage
    showExpression("normal");
...
function handleItemAction(itemName, itemPrice, itemType, sudahDibeli) {
    ...
    if (sudahDibeli) {
        ...
        if (typeof coins !== 'undefined' && coins >= itemPrice) {
            ...
            let koleksi = JSON.parse(sessionStorage.getItem("koleksiItem")) || {}; // DIUBAH KE sessionStorage
            if (!koleksi[itemType]) {
                koleksi[itemType] = [];
            }
            if (!koleksi[itemType].includes(itemName)) {
                koleksi[itemType].push(itemName);
                sessionStorage.setItem("koleksiItem", JSON.stringify(koleksi)); // DIUBAH KE sessionStorage
            }
            pakaiItem(itemName, itemType);
            bounceOnce();
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function generateItems(tipe) {
        ...
        const koleksi = JSON.parse(sessionStorage.getItem("koleksiItem")) || {}; // DIUBAH KE sessionStorage
