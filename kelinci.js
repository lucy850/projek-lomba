...
// --- PERBAIKAN DI SINI: Variabel ekspresi diubah untuk kelinci ---
const ekspresiRabbit = {
    normal: "kelinci.png",
    love: "kelinci love.png",
    sad: "kelinci sedih.png"
};

// --- PERBAIKAN DI SINI: Nama item diubah menjadi 'Rabbit' ---
const itemData = {
    hair: [
        { name: "Rabbit Long Hair", price: 30, image: "long kelinci.png" },
        { name: "Rabbit Curly Hair", price: 30, image: "curly kelinci.png" },
        { name: "Rabbit Short Hair", price: 30, image: "short kelinci.png" }
    ],
    accessories: [
        { name: "Rabbit Scarf", price: 30, image: "syal kelinci.png" },
        { name: "Rabbit Hat", price: 30, image: "hat kelinci.png" },
        { name: "Rabbit Necklace", price: 30, image: "kalung kelinci.png" }
    ],
    playTools: [
...
function showExpression(type, duration = 0) {
    ...
    const terpasang = JSON.parse(sessionStorage.getItem("itemTerpasang")) || {};
    let gambarFinal = ekspresiRabbit[type]; // Menggunakan ekspresiRabbit yang sudah diperbaiki
...
function lepasItem(tipe) {
    ...
    sessionStorage.setItem("itemTerpasang", JSON.stringify(terpasang));
    showExpression("normal");
...
function handleItemAction(itemName, itemPrice, itemType, sudahDibeli) {
    ...
    if (sudahDibeli) {
        ...
        if (typeof coins !== 'undefined' && coins >= itemPrice) {
            ...
            let koleksi = JSON.parse(sessionStorage.getItem("koleksiItem")) || {};
            if (!koleksi[itemType]) {
                koleksi[itemType] = [];
            }
            if (!koleksi[itemType].includes(itemName)) {
                koleksi[itemType].push(itemName);
                sessionStorage.setItem("koleksiItem", JSON.stringify(koleksi));
            }
            pakaiItem(itemName, itemType);
            bounceOnce();
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function generateItems(tipe) {
        ...
        const koleksi = JSON.parse(sessionStorage.getItem("koleksiItem")) || {};
